...
export class ManifestAssetsPluginDist {
  ...
  apply(compiler: webpack.Compiler) {
    ...
    compiler.hooks.afterEmit.tapAsync('vexa-manifest-plugin-dist', async (compilation, next) => {
      const result: {
        jsModule: string;
      ...
      await fs.writeJson(this.output, result, { spaces: 2 });
      next();
    });
  }
}
